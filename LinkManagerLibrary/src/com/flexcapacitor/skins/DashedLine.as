...
package com.flexcapacitor.skins {
	...
	public class DashedLine 
	{
		...
		// how far the lines extend past the edge when drawing a rectangle
		public var edgeOffset:Number = 1;
		public var sides:Boolean = true;
		...
		// public methods
		/**
		 * Draw rectangle using dashed lines
		 * 
		 * addChild(newSprite);
		 * var dashedLine:DashedLine = new DashedLine(newSprite, 0, .5);
		 * dashedLine.lineStyle(0, 0x000000, 1.9);
		 * dashedLine.drawRectangle(250, 250, 100, 50, .4, 1.6);
		 */
		public function drawRectangle(x:Number, y:Number, width:Number, height:Number, onLength:Number = -1, offLength:Number = -1):void {
			if (onLength==-1) onLength = this.onLength;
			if (offLength==-1) offLength = this.offLength;
			
			// if its blank you probably need to set the line style
			//dashedLine.lineStyle(.5, 0x000000, 1);
			
			var dashedLine:DashedLine = new DashedLine(target, onLength, offLength);
			var x1:Number = x;
			var y1:Number = y;
			var w1:Number = x1 + width;
			var h1:Number = y1 + height;
/*			dashedLine.moveTo(x1, y1);
			dashedLine.lineTo(w1, y1);
			dashedLine.lineTo(w1, h1);
			dashedLine.lineTo(x1, h1);
			dashedLine.lineTo(x1, y1);*/
			
			
			dashedLine.moveTo(x1-edgeOffset, y1);
			dashedLine.lineTo(w1+edgeOffset, y1); // top left to top right
			dashedLine.moveTo(w1, y1-edgeOffset);
			if (sides) dashedLine.lineTo(w1, h1+edgeOffset); // top right to bottom right
			dashedLine.moveTo(w1+edgeOffset, h1);
			dashedLine.lineTo(x1-edgeOffset, h1); // bottom right to bottom left
			dashedLine.moveTo(x1, h1+edgeOffset);
			if (sides) dashedLine.lineTo(x1, y1-edgeOffset); // left to top
